import inspect
from typing import Any, Iterator, List, Optional, Tuple, Type, no_type_check
...
class State:
    ...
    def set_parent(self, group: Type["StatesGroup"]) -> None:
        if not issubclass(group, StatesGroup):
            raise ValueError("Group must be a subclass of StatesGroup")
        self._group = group

    def match_state(self, state: Optional[str] = None) -> bool:
        if self._state == "*":
            return True
...
class StatesGroupMeta(type):
    ...
    def __new__(msc, name, bases, namespace, **kwargs):
        cls = super().__new__(msc, name, bases, namespace, **kwargs)
...
class StatesGroup(metaclass=StatesGroupMeta):
    ...
    def match_state(self, state: Optional[str] = None) -> bool:
        return state in type(self).__all_states_names__
